...
<div class="submit">       
<button onclick="saveEditedTaskData(${taskId})" class="createTask">Save <img src="/assets/img/check.svg" alt=""></button>
</div>
`;
...
function saveEditedTaskData(taskId) {
  ...
  const contactCheckboxes = document.querySelectorAll('#options .option input[type="checkbox"]');
  ...
  // Kategorie
  const selectedCategory = document.querySelector('.category-select .selected-option').textContent.trim();
  task.category = {
  ...
  // Subtasks
  const subtaskCheckboxes = document.querySelectorAll('.subtasks-list input[type="checkbox"]');
